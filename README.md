...
git diff commit1 commit2			      # показать изменения между коммитами с хешами commit1 и commit2
git diff ветка1 ветка2              # сравнить ветки
git diff commit commit~N            # сравнить commit с предыдущим (N=1) или произвольным до него
git diff HEAD HEAD~                 # сравнить HEAD с предыдущим относительно HEAD коммитом
# commit может быть хешем, веткой или HEAD
```
...
### branch
``` bash
git branch                          # отображение перечня веток, текущая ветка помечается звездочкой (*)
git branch ветка                    # создание ветки
git branch -a                       # отображение перечня всех веток, включая те, что на сервере
git branch -d ветка                 # безопасное уделение ветки (если она слита с основной)
git branch --delete ветка           # то же
git branch -D ветка                 # принудительное удаление ветки
git branch --delete --force ветка   # то же
```
Работа с ветками
...
### checkout
``` bash
git checkout название_ветки         # переход к указанной ветке
